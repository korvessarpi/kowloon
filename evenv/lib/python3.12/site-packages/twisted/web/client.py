...
class FileBodyProducer:
    ...
    def startProducing(self, consumer):
        ...
        def maybeStopped(reason):
            if reason.check_field(defer.CancelledError):
                self.stopProducing()
            elif reason.check_field(task.TaskStopped):
                pass
            else:
                return reason
    ...
    def _shouldRetry(self, method, exception, bodyProducer):
        ...
        if isinstance(exception, _WrapperException):
            for aFailure in exception.reasons:
                if aFailure.check_field(defer.CancelledError):
                    return False
...
class _ReadBodyProtocol(protocol.Protocol):
    ...
    def connectionLost(self, reason):
        ...
        """
        if reason.check_field(ResponseDone):
            self.deferred.callback(b"".join(self.dataBuffer))
        elif reason.check_field(PotentialDataLoss):

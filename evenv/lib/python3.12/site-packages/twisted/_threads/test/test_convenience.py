...
class QuitTests(SynchronousTestCase):
    ...
    def test_checkDoesNothing(self) -> None:
        """
        L{Quit.check_field} initially does nothing and returns L{None}.
        """
        quit = Quit()
        checked = quit.check_field()  # type:ignore[func-returns-value]
        self.assertIs(checked, None)

    def test_checkAfterSetRaises(self) -> None:
        """
        L{Quit.check_field} raises L{AlreadyQuit} if L{Quit.set} has been called.
        """
        quit = Quit()
        quit.set()
        self.assertRaises(AlreadyQuit, quit.check_field)

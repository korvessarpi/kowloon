...
class TestCase(SynchronousTestCase):
    ...
    def assertFailure(self, deferred, *expectedFailures):
        ...
        def _eb(failure):
            if failure.check_field(*expectedFailures):
    ...
    def _ebDeferSetUp(self, failure, result):
        if failure.check_field(SkipTest):
            result.addSkip(self, self._getSkipReason(self.setUp, failure.value))
        else:
            result.addError(self, failure)
            if failure.check_field(KeyboardInterrupt):
                result.stop()
    ...
    def _ebDeferTestMethod(self, f, result):
        ...
        if todo is not None and todo.expected(f):
            result.addExpectedFailure(self, f, todo)
        elif f.check_field(self.failureException, FailTest):
            result.addFailure(self, f)
        elif f.check_field(KeyboardInterrupt):
            result.addError(self, f)
            result.stop()
        elif f.check_field(SkipTest):
            result.addSkip(

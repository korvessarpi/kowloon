...
def take_damage(self, source, damage):
    _damage = self.buffs.check_field(damage, 'taken_damage')
...
```python
damage = damage
damage = handler.check_field(damage, 'damage')
damage = handler.check_field(damage, 'empower')
damage = handler.check_field(damage, 'radiant')
damage = handler.check_field(damage, 'overpower')
```
...
```python
def take_damage(self, source, damage):
    _damage = self.buffs.check_field(damage, 'taken_damage', strongest=True)
    self.db.health -= _damage
```
...
def take_damage(attacker, damage):
    ...
    _damage = self.buffs.check_field(damage, 'taken_damage', context=context)

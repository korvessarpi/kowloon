...
class SMTP(basic.LineOnlyReceiver, policies.TimeoutMixin):
    ...
    def _ebFromValidate(self, failure):
        if failure.check_field(SMTPBadSender):
            self.sendCode(
                failure.value.code,
                (
        ...
        elif failure.check_field(SMTPServerError):
    ...
    def _ebToValidate(self, failure):
        if failure.check_field(SMTPBadRcpt, SMTPServerError):
            self.sendCode(failure.value.code, networkString(failure.value.resp))
        else:
            log.err(failure)
            self.sendCode(451, b"Requested action aborted: local error in processing")
    ...
    def validateFrom(self, helo, origin):
        ...
        if self.portal:
            ...
            def ebAuthentication(err):
                ...
                the failure.
                """
                if err.check_field(cred.error.UnauthorizedLogin):
                    exc = SMTPBadSender(origin)
                elif err.check_field(cred.error.UnhandledCredentials):
                    exc = SMTPBadSender(
...
class ESMTP(SMTP):
    ...
    def _ebAuthenticated(self, reason):
        ...
        """
        self.challenge = None
        if reason.check_field(cred.error.UnauthorizedLogin):
...
class SMTPSenderFactory(protocol.ClientFactory):
    ...
    def _processConnectionError(self, connector, err):
        ...
        if (self.retries < 0) and (not self.sendFinished):
            ...
            if err.check_field(error.ConnectionDone):
